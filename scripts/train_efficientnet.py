...
from src.models.efficientnet_b0 import count_parameters
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=224,
        help="Image size for training (EfficientNet-B0 expects 224x224)",
    )
    parser.add_argument(
        "--num_workers",
        type=int,
    ...
    parser.add_argument(
        "--no_pretrained",
        action="store_true",
        help="Disable ImageNet pretraining for EfficientNet-B0",
    )
    ...
    # TensorBoard writer
    writer = SummaryWriter(log_dir=str(log_dir / "efficientnet_b0"))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % 10 == 0:
            checkpoint_path = save_dir / f"efficientnet_b0_epoch_{epoch + 1}.pth"
            save_checkpoint(
    ...
    # Save test results
    results_file = results_dir / "efficientnet_b0_test_results.txt"
    with open(results_file, "w") as f:
        f.write("EfficientNet-B0 - Test Set Results\n")
        f.write("=" * 60 + "\n")
    ...
    plt.tight_layout()
    plt.savefig(
        results_dir / "efficientnet_b0_training_curves.png",
        dpi=150,
        bbox_inches="tight",
    )
    print(
        f"Training curves saved to {results_dir / 'efficientnet_b0_training_curves.png'}"
    )
    plt.close()
